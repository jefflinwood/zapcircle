...
export function findBehaviorForComponent(componentFilePath: string): string | undefined {
    const componentDir = path.dirname(componentFilePath);
    const componentBasename = path.basename(componentFilePath, path.extname(componentFilePath));
    
    const behaviorFiles = fs.readdirSync(componentDir).filter(file => file.endsWith('.zap.toml'));
