...
const walkImports = async (entryFile: string, depthLimit: number = Infinity, visited: Set<string> = new Set()): Promise<string[]> => {
    ...
    traverse(ast, {
        ImportDeclaration({ node }) {
            ...
            if (!importPath.startsWith('.') || importPath.includes('node_modules')) return;
